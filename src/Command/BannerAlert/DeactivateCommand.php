...
class DeactivateCommand extends Command
{
    ...
    {
        $this
            ->setName('app:banner-alerts:deactivate')
            ->setDescription(
                'Deactivates banner alerts.'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $inactiveAlerts = $repo->findBy(['active' => true], ['id' => 'desc']);
        ...
        if (empty($options)) {
            $output->writeln('No banner alerts to deactivate found.');
        } else {
            $helper = $this->getHelper('question');
            $question = new ChoiceQuestion(
                'Please select which banner alerts to deactivate: ',
                $options
            );
            ...
            if (1 === count($choices)) {
                ...
                $output->writeln('The following banner alerts have been deactivated: ' . implode(', ', $choices) . '.');
            }
        }
